...
IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_RESPTEAMS')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_RESPTEAMS nvarchar(max)
  END
  IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_RESPUSERS')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_RESPUSERS nvarchar(max)
  END
  IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_IMPACTEDSRV')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_IMPACTEDSRV nvarchar(max)
  END
  IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCWEBFORM')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCWEBFORM nvarchar(1000)
  END
  IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCAPILINK')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCAPILINK nvarchar(2000)
  END
   IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCTINYID')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCTINYID nvarchar(10)
  END
   IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCCREATEDT_UT')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCCREATEDT_UT datetime
  END
   IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCLASUPDDT_UT')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCLASUPDDT_UT datetime
  END
   IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_INCIMPACTSTARTDT_UT')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
  E_OPSG_INCIMPACTSTARTDT_UT datetime
  END
  IF NOT EXISTS ( SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[40000].[SD_REQUEST]') AND name = 'E_OPSG_TAGS')
  BEGIN
alter table [EVO_DATA40000].[40000].[SD_REQUEST] add  
...
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_TRUNC_TEMPDATA]' ) AND Type IN (N'P', N'PC'))
BEGIN
DROP PROCEDURE [40000].[E_OPSG_TRUNC_TEMPDATA]
END
GO

CREATE PROCEDURE [40000].[E_OPSG_TRUNC_TEMPDATA]  
AS 
begin 
set nocount on 
...
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_JSON_PARSE]' ) AND Type IN (N'P', N'PC'))

BEGIN

DROP PROCEDURE [40000].[E_OPSG_JSON_PARSE]

END
GO

CREATE PROCEDURE [40000].[E_OPSG_JSON_PARSE] (@jsonData nvarchar(max), @guid uniqueidentifier) 
...
END
GO
-- note : call with EXEC [40000].[E_OPSG_JSON_PARSE] @jsonData , @guid

-- SProc to parse Service Array into Temp Service table
use [EVO_DATA40000] 
go 
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_SERVICES_PARSE]' ) AND Type IN (N'P', N'PC'))
BEGIN
DROP PROCEDURE [40000].[E_OPSG_SERVICES_PARSE]
END
GO
CREATE PROCEDURE [40000].[E_OPSG_SERVICES_PARSE] (@incID nvarchar(255), @jsonData nvarchar(max))
AS 
begin 
set nocount on 
...
go
-- note : call with EXEC [40000].[E_OPSG_SERVICES_PARSE] <incID>, <impacted service table as json>

-- SProc to parse Responders array into Temp Responders Table
use [EVO_DATA40000] 
go 
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_RESPONDERS_PARSE]' ) AND Type IN (N'P', N'PC'))
BEGIN
DROP PROCEDURE [40000].[E_OPSG_RESPONDERS_PARSE]
END
GO
CREATE PROCEDURE [40000].[E_OPSG_RESPONDERS_PARSE] (@incID nvarchar(255), @jsonData nvarchar(max))
AS 
begin 
set nocount on 
...
go
-- note : call with EXEC [40000].[E_OPSG_RESPONDERS_PARSE] <incID>, <responders table as json>
...
GO
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_SRV_PARSE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_SRV_PARSE]
END
GO

CREATE FUNCTION [40000].[E_OPSG_SRV_PARSE] (@json nvarchar(max))
RETURNS TABLE 
AS
RETURN
...
GO
-- note : Call with SELECT top 1 serviceName from [40000].[E_OPSG_SRV_PARSE] (jsonstring)

-- Function to parse TeamName from Team Get Json
USE [EVO_DATA40000]
GO

IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_TEAM_PARSE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_TEAM_PARSE]
END 
GO
...
CREATE FUNCTION [40000].[E_OPSG_TEAM_PARSE] (@json nvarchar(max))
RETURNS TABLE 
AS
RETURN
...
GO
-- note : call with SELECT top 1 name from [40000].[E_OPSG_TEAM_PARSE] (@json)

-- Function to parse userName from User Get Json
USE [EVO_DATA40000]
GO
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_USER_PARSE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_USER_PARSE]
END 
GO
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [40000].[E_OPSG_USER_PARSE] (@json nvarchar(max))
RETURNS TABLE 
AS
RETURN
...
GO
-- note : call with SELECT top 1 userName from [40000].[E_OPSG_USER_PARSE] (@json)

-- Function to concat impacted services
USE [EVO_DATA40000]
GO

IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_SRVTABLE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_SRVTABLE]
END
GO
...
CREATE FUNCTION [40000].[E_OPSG_SRVTABLE] (@requestID int) 
...
BEGIN
    ...
    DECLARE srvCursor CURSOR FOR SELECT serviceLabel  from 
	  [EVO_DATA40000].[40000].sd_request sdr
...
END
GO
-- note : call with select [40000].[E_OPSG_SRVTABLE] (requestID)

-- Function to concat users and teams
USE [EVO_DATA40000]
GO
IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_RESPTABLE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_RESPTABLE]
END
GO
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [40000].[E_OPSG_RESPTABLE] (@requestID int, @respType nvarchar(30)) 
RETURNS NVarchar(4000) 
as
BEGIN
	declare @table nvarchar(4000)
...
END
GO
-- note : call with select [40000].[E_OPSG_RESPTABLE] (requestID, respType)

-- Function to concat tags
USE [EVO_DATA40000]
GO

IF EXISTS( SELECT 1 FROM sys.Objects WHERE object_ID = OBJECT_ID(N'[40000].[E_OPSG_TAGTABLE]' ) AND Type IN (N'FN', N'IF', N'TF', N'FS', N'FT'  ) )
BEGIN
DROP FUNCTION [40000].[E_OPSG_TAGTABLE]
END
GO
...
CREATE FUNCTION [40000].[E_OPSG_TAGTABLE] (@requestID int) 
...
END
GO
-- notes : call with select [40000].[E_OPSG_TAGTABLE] (requestID)
